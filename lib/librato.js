...
{
  if (debug) { util.log('Librato payload = ' + payload); }
...
var flush_stats = function librato_flush(ts, metrics)
{
  ...
  var addMeasure = function add_measure(mType, measure, countStat) {
    ...
    // check if the metric name has source value(s) included after '/' separators, and
    // split it out into separate source/name values for transmission to Librato Metrics.
    // The format is:
    //   metric.name/mainsource
    //   metric.name/suffix=mainsource
    //   metric.name/mainsource/suffix=secondarysource
    //   metric.name/mainsource/suffix=secondarysource/suffix2=tertiarysource
    //   etc.
    // Examples:
    //   main.requests/us
    //   main.requests/us/by-language=english/by-gender=male
    //
    // The first example will only send one measurement to librato with the name set to "main.requests"
    // and source set to "us".  The second example will do the same, but will also send a copy
    // with the metric name "main.requests:by-language" and source "english",
    // plus another with the metric name "main.requests:by-gender" and source "male"
    // You may omit specifying a suffix for at most one of the sources.  Make sure no suffix is duplicated
    // or you'll be sending duplicate measurements for the same metric.
    //
    // Since statsd doesn't support ':' in a measurement name but librato does, we use ';' instead and convert
    // it here.
    var metricName = measure.name;
    if (debug) {
      metricName = '_test.' + metricName;
    }
    if (metricName.indexOf('/') > 0 && metricName.indexOf('/') < metricName.length - 1) {
      var sources = metricName.split('/');
      var name = sources.shift();
      for (var skey in sources) {
        var source = sources[skey].replace(';', ':');
        var suffix = '';
        if (source.indexOf('=') > 0 && source.indexOf('=') < source.length - 1) {
          suffix = ':' + sanitize_name(source.split('=', 1)[0]);
          source = source.slice(suffix.length);
        }

        var newMeasure = extend({}, measure);
        newMeasure.source = sanitize_name(source);
        newMeasure.name = sanitize_name(name) + suffix;
        measures.push(newMeasure);
      }
    }
    else {
      measure.name = sanitize_name(metricName);
      measures.push(measure);
    }

    for (var mkey in measures) {
      if (mType == 'counter') {
        counters.push(measures[mkey]);
      } else {
        gauges.push(measures[mkey]);
      }
